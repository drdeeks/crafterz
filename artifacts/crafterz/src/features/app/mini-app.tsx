...
import { postGmEvent, postMintEvent } from './runtime-api';
import type { AppInventoryItem, AppTab } from './app-types';
import { TIER_BADGE, EVM_CHAINS, CRAFTZ_COST, PTS } from './constants';
...
import { InventoryTab, MegaMindsTab, TasksTab, LeaderboardTab, AdminTab } from './tabs';
...
export function MiniApp() {
  ...
  function handleClaimTask(taskId: string) {
    claimTask(taskId, (task) => {
      ...
      setCraftz((c) => { const next = Math.min(100, c + task.craftzReward); craftzRef.current = next; return next; });
    });
  }
