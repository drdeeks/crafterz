...
export function toDiscoveryFeed(events: ServerActivity[]): DiscoveryFeedItem[] {
  const mintedKeys = new Set(
    events
      ...
      .map(
        (event) =>
          `${normalizeKeyPart(event.username)}:${normalizeKeyPart(event.metadata?.itemName)}`,
      ),
  );
    ...
    .map((event) => {
      ...
      return {
        ...
        emojis: (eventEmojis.length > 0
          ...
          : ["✨"]) as [string, string?],
        tier,
        discoverer: event.username,
        time: getRelativeTime(event.timestamp),
        minted: mintedKeys.has(
          `${normalizeKeyPart(event.username)}:${normalizeKeyPart(itemName)}`,
        ),
      };
    });
}
