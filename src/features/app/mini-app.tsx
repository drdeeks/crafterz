...
// ─── Admin config ─────────────────────────────────────────────────────────────
// In production, these should be derived from the authenticated Farcaster session.
// For development, defaults are provided. Set via window.__CRAFTERZ_ADMIN_FID if needed.
const _w = typeof window !== "undefined" ? (window as unknown as Record<string, unknown>) : {};
const ADMIN_FID = _w.__CRAFTERZ_ADMIN_FID ? Number(_w.__CRAFTERZ_ADMIN_FID) : 99999;
const CURRENT_USER_FID = _w.__CRAFTERZ_USER_FID ? Number(_w.__CRAFTERZ_USER_FID) : 99999;
const CURRENT_USER_USERNAME = _w.__CRAFTERZ_USERNAME ? String(_w.__CRAFTERZ_USERNAME) : "you.fid";
...
// Contains items that are pre-discovered (exist in LOGICAL_COMBINATIONS or
// can be produced by category templates). Items not producible by the current
// crafting system are excluded to prevent unreachable registry entries.
const GLOBAL_REGISTRY = new Set<string>([
  'steam', 'lava', 'mud', 'mist', 'dust', 'plasma',
  'cooked', 'hydrated', 'plant', 'bird',
]);
...
export function MiniApp() {
  ...
  const refreshServerSnapshot = useCallback(async () => {
    ...
    const currentPlayer = snapshot.leaderboard.find(
      (player) => player.fid === CURRENT_USER_FID || player.agentId === `fid:${CURRENT_USER_FID}`,
    );
    ...
    <div className="h-dvh flex flex-col overflow-hidden bg-zinc-950 text-white" style={{ fontFamily: 'var(--font-geist-sans, system-ui, sans-serif)' }}>
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-zinc-900 border-b border-zinc-800 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <img src={`https://api.dicebear.com/9.x/lorelei/svg?seed=${CURRENT_USER_USERNAME}`} className="w-7 h-7 rounded-full border border-zinc-700" alt="" />
        </div>
      </div>
