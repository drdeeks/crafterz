...
import { getX402Server, getFacilitatorUrl, NETWORKS, PRICES, WALLET_ADDRESSES } from "./config";
import type { x402ResourceServer, X402PaymentRequest, X402PaymentResult } from "@x402/core/server";
...
/**
 * Convert USD string price to bigint (wei/lamports based on network)
 * Note: This is a utility for display/conversion, actual payment amounts are handled by x402 SDK
 */
export function priceToBigInt(price: string, decimals: number = 18): bigint {
  const usdValue = parseFloat(price.replace("$", ""));
  // For now, assume 1 USD = 1 token (actual conversion depends on oracle)
  // In production, use actual token price from oracle
  const tokenAmount = usdValue;
  const divisor = 10n ** BigInt(decimals);
  return BigInt(Math.round(tokenAmount * Number(divisor)));
}
...
// Export types
export type { X402Config, PaymentAction as X402PaymentAction } from "./config";
export type { CrafterZAction, X402VerificationResult };
