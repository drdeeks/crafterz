...
export async function POST(request: NextRequest) {
  ...
  if (address && data.txHash && data.chainId) {
    const txVerified = await verifyTxOnChain(data.txHash, data.chainId, address);
    ...
    if (txVerified) {
      ...
      if (isWhitelisted) {
        ...
        const verificationResult = await verifyX402Payment(data.txHash as `0x${string}`, x402Amount || 0n, data.chainId, address as `0x${string}`);
        x402Paid = verificationResult.verified;
        x402Protocol = "X402";
      }
    }
