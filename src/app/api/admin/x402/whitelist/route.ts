...
const addressSchema = z.union([
  ...
  z.string().length(44).regex(/^[1-9A-HJ-NP-Za-km-z]+$/, "Invalid Solana address"), // Base58
]);
...
export async function GET(request: NextRequest) {
  ...
  const ownerAddress = request.headers.get("x-address");
  
  if (!ownerAddress || !isOwner(ownerAddress)) {
