...
export async function GET(request: NextRequest) {
  ...
  const agentId: AgentID = agentType === "farcaster" ? (parseInt(fid ?? "0") || fid || "0") : fid || "0";
  const tasks = await getDailyTasks(agentId, agentType, dateKey ?? undefined);
...
export async function POST(request: NextRequest) {
  ...
  // Extract agent info from headers (set by middleware)
