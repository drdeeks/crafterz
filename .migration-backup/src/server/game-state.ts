...
import { createPublicClient, http, type Hash, type Address, type Chain } from "viem";
...
async function saveLeaderboardStore(store: Record<string, PlayerStats>) {
  ...
  const oldFormatStore: Record<string, LegacyPlayerRecord> = {};
  ...
  type SerializablePlayer = Omit<PlayerStats, "x402Spent"> & { x402Spent: string };
  const serializableStore: Record<string, SerializablePlayer> = {};
  for (const [key, player] of Object.entries(store)) {
...
async function pushActivity(event: ActivityEvent) {
  ...
  try {
    ...
    console.error("Failed to write activity v2:", error);
  }

  // Push to v1 format (legacy compatibility)
  try {
    ...
    console.error("Failed to write activity v1:", error);
  }
}

// Storage keys for KV store
const STORAGE_KEYS = {
  leaderboard: "craftz:leaderboard",
  leaderboardV2: "craftz:leaderboard:v2",
  activity: "craftz:activity",
  activityV2: "craftz:activity:v2",
} as const;

// Maximum number of activity items to store
const MAX_ACTIVITY_ITEMS = 1000;

// Pool of possible target items for daily tasks
...
export async function recordCraft(input: {
  ...
  const x402Amount = input.x402Amount || 0n;
...
export async function recordGm(input: {
  ...
  let x402Verified = false;
  const x402Amount = input.x402Amount || 0n;
  const effectiveChainId = input.chainId || mapChainStringToId(input.chain);
  let finalX402Amount = 0n;
